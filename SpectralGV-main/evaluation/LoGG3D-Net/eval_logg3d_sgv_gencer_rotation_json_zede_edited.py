...
class MetLocEvaluator(Evaluator):
    ...
    def evaluate(self, model, d_thresh,min_num_feat,MEAN_SHIFT_p ,path_model_D3Feat,D3Feat,name_file_exel,name_descripteur_global,name_file_visu,reset=False,file_rotation='',visualisation = False ,rayon =3,nb_clusteur = 50, bandwidth = 2 ,utilisation_exel=True,*args, **kwargs):
        ...
        for ndx, e in tqdm.tqdm(enumerate(self.eval_set.query_set)): # global descriptor loading
        ...
        for query_ndx in tqdm.tqdm(query_indexes): #5031
                ...
                for k in range(topk):
                    ...
                    if D3Feat == True:
                        ...
                        if visualisation == True  :
                            ...
                            print("fitness list 11")
                    else : 
                        if k == 0 :
                            local_query_embeddings_LoGG3D = self.compute_embeddings(self.eval_set.query_set, model, id_nom =id_query) # (list, logg3dnet model, 0)
                        ...
                        if visualisation == True  :
                            ...
                            print("fitness list 3")

                            
                    fitness_list[k] = conf_val
                print(fitness_list)
                
                topk_rerank = np.flip(np.asarray(fitness_list).argsort()) # array([0, 1, 2, 3, 7, 9, 8, 5, 6, 4])
                topk_rerank_inds = copy.deepcopy(nn_ndx) 
                topk_rerank_inds[:topk] = nn_ndx[topk_rerank] # array([   0,    1,   27,   26,   75, 4466, 1276, 1227, 4576, 1302])
                t_rerank = time() - tick # 3.1296815872192383
...
if __name__ == "__main__":
    ...
    args.weights =  "/lustre/fswork/projects/rech/dki/ujo91el/code/tool_lidar_hd/LoGG3D-Net/training/checkpoints/2025-05-25_14-12-46_run_0_3"
    print(f'weights: {args.weights}')
