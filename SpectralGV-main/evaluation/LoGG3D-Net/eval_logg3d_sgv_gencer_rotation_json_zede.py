...
class Evaluator:
    ...
    def __init__(self, dataset_root: str, dataset_type: str, eval_set_pickle: str, device: str, json_index : str , 
        ...
        self.eval_set_filepath = "/lustre/fswork/projects/rech/dki/ujo91el/code/SpectralGV_D3Feat/SpectralGV-main/datasets/lidar/file_traitment.pickle"
    ...
    def compute_embedding(self, pc, model, *args, **kwargs):
        # This method must be implemented in inheriting classesining/checkpoints/2025-05-25_14-12-46_run_0_3
...
class MetLocEvaluator(Evaluator):
    ...
    def evaluate(self, model, d_thresh,min_num_feat,MEAN_SHIFT_p ,path_model_D3Feat,D3Feat,name_file_exel,name_descripteur_global,name_file_visu,reset=False,file_rotation='',visualisation = False ,rayon =3,nb_clusteur = 50, bandwidth = 2 ,utilisation_exel=True,*args, **kwargs):
        ...
        directori_json = '/lustre/fsn1/worksf/projects/rech/dki/ujo91el/datas/lidarhd_v2/rotation/rotation.json'
    ...
    def evaluate2(self, model, d_thresh,min_num_feat,MEAN_SHIFT_p ,path_model_D3Feat,D3Feat,name_file_exel,name_descripteur_global,name_file_visu,reset=False,file_rotation='',visualisation = False ,rayon =3,nb_clusteur = 50, bandwidth = 2 ,utilisation_exel=True,*args, **kwargs):
        ...
        path = "/lustre/fsn1/worksf/projects/rech/dki/ujo91el/datas/lidarhd_v2/dsi_train_list.json"
        with open(path, "r") as f:
            train = json.load(f)
        print(len(train))
        traind = {path: i for i, path in enumerate(train)}
        
        path = "/lustre/fswork/projects/rech/dki/ujo91el/code/DSI_LIDAR_HD/dictio_results/results_dsi3d_east_global.json"
        #path = "/lustre/fswork/projects/rech/dki/ujo91el/code/DSI_LIDAR_HD/dictio_results/results_dsi3d_moe_est_global3.json"
        with open(path, "r") as f:
        ...
        if d_thresh > 0:
            ...
            for i in tqdm.tqdm(results_dict):
                ...
                try:
                    ...
                    # recenter
                    keypoints = local_query_embeddings_LoGG3D['keypoints']  # (N, 3)
                    query_xy = torch.tensor(
                        query_positions[id_query],
                        device=keypoints.device,
                        dtype=keypoints.dtype
                    )  # shape (2,)
                    
                    # subtract from x,y only
                    keypoints_xy_centered = keypoints.clone()
                    keypoints_xy_centered[:, :2] -= query_xy
                    
                    local_query_embeddings_LoGG3D['keypoints'] = keypoints_xy_centered

                    query_xy_np = query_positions[id_query] 

                    coord_D3Feat_query[:, :2] -= query_xy_np
                ...
                for k_id in top_k:
                    ...
                    try: 
                        local_map_embeddings_LoGG3D = self.compute_embeddings(self.eval_set.map_set, model, id_nom = k_id) 
                        #coord_D3Feat_map = self.D3feat_fonction(self.eval_set.map_set , k_id , file_rotation ,nb_clusteur, MEAN_SHIFT_p , bandwidth) 
                        #coord_D3Feat_map = self.D3feat_fonction(self.eval_set.query_set , id_query , file_rotation ,nb_clusteur, MEAN_SHIFT_p , bandwidth) 
           
                        
                        #try:
                        # recenter
                        keypoints2 = local_map_embeddings_LoGG3D['keypoints']  # (N, 3)
                        query_xy2 = torch.tensor(
                            query_positions[k_id],
                            device=keypoints.device,
                            dtype=keypoints.dtype
                        )  # shape (2,)
                        
                        # subtract from x,y only
                        keypoints_xy_centered2 = keypoints2.clone()
                        keypoints_xy_centered2[:, :2] -= query_xy2
                        
                        local_map_embeddings_LoGG3D['keypoints'] = keypoints_xy_centered2
    
                        coord_D3Feat_map = coord_D3Feat_query
        ...
        if id_nom == None:
            for ndx, e in tqdm.tqdm(enumerate(eval_subset)):
                ...
                if not os.path.exists(chemin_fichier) or reset== True : 
                    ...
                    #hash_vals, coords, indices, input , points = make_sparse_tensor(xyzi, voxel_size=self.voxel_size, num_points=200000,return_points=True, return_hash = True)
                    hash_vals, coords, indices, input , points = make_sparse_tensor(xyzi, voxel_size=2, num_points=200000,return_points=True, return_hash = True)
    ...
    def D3feat_fonction(self ,eval_subset,id_nom ,file_rotation,nb_clusteur,MEAN_SHIFT_p=False ,bandwidth = 2 ): 
        ...
        src_data = np.load(os.path.join(chemain_stokage_fichier ,f"descripteur_D3Feat/lidarhd_v2_2m/{src_name}.npz"))
        #import pdb; pdb.set_trace()
