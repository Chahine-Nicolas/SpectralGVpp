...
class D3Feat:
    ...
    def __init__(self, model_path, dataset_root, eval_set_query, eval_set_map, MEAN_SHIFT_p=False, voxel_size=0.3, reset=False, file_rotation='',debut=0):
        ...
        self.model_path = os.path.join(self.env_path, model_path) # '/lustre/fswork/projects/rech/dki/ujo91el' / 'SpectralGV_D3Feat/models/D3Feat'
    ...
    def run(self):
        ...
        ply_directory = self.eval_set_query[0]

        # ref
        # self.dataset_root = '/lustre/fsn1/worksf/projects/rech/dki/ujo91el/datas/datasets/sequences/00/'
        # self.dataset_bin_root = '/lustre/fsn1/worksf/projects/rech/dki/ujo91el/datas/datasets/sequences/00/velodyne'    
        self.dataset_bin_root = self.dataset_root + 'velodyne'
        # self.dataset_ply_root = '/lustre/fsn1/worksf/projects/rech/dki/ujo91el/datas/datasets/sequences/00/ply'
        self.dataset_ply_root = self.dataset_root + 'ply'
        
        #ply_path = os.path.join(self.env_path, self.dataset_root, str(ply_directory) + "/ply")
        #ply_path = os.path.join(self.dataset_root, str(ply_directory) + "/ply")
        ply_path = self.dataset_ply_root
        ...
        for ndx, e in tqdm(enumerate(self.eval_set_query)):
            ...
            last_element = os.path.basename(e)
            ...
            if not os.path.exists(ply_file_map) or self.reset == True:
                ...
                self.bin_to_ply_with_intensity(os.path.join(self.env_path, self.dataset_bin_root, e+".bin"), ply_file_map)

            
            #name_point_cloud_files.append(os.path.join(self.env_path, ply_path, os.path.basename(e.rel_scan_filepath)[:-4] + ".ply"))
            name_point_cloud_files.append(ply_file_map)
        ...
        for i in tqdm(range(self.debut, len(name_point_cloud_files), 1)):
            ...
            import pdb; pdb.set_trace()
            tester.generate_descriptor(model, dataset, self.folder_rotation) # kpconv, dataset, '/lustre/fsn1/projects/rech/dki/ujo91el/descripteur_D3Feat/rotation'
...
class EvaluationSet:
    ...
    def load(self, pickle_filepath: str):
        ...
        dict_link = "/lustre/fswork/projects/rech/dki/ujo91el/code/DSI-3D/results_dsi3d_00_hilbert.json"
        with open(dict_link, "r") as f: 
            a = json.load(f)
        query_l, map_l = [  '%06d' % a[0]['query_idx']], a[0]['Top10_id']
        ...
        self.query_set = []
        for e in query_l:
            self.query_set.append(e)
        self.map_set = []
        for e in map_l:
            self.map_set.append(e) 
        """
        OG
        for e in query_l:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_type', type=str, required=False, default='kitti360', choices=['mulran', 'southbay', 'kitti', 'alita', 'kitti360','lidar'])
