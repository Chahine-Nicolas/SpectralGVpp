...
"""
################# modifier ########################
class EvaluationTuple_lidar:
    # Tuple describing an evaluation set element
    def __init__(self, rel_scan_filepath: str, position: np.array):
        # position: x, y position in meters
        # pose: 6 DoF pose (as 4x4 pose matrix)
        assert position.shape == (2,)
        self.rel_scan_filepath = rel_scan_filepath
    ...
    def to_tuple(self):
        return self.rel_scan_filepath, self.position
"""
...
"""       
###################### modifier ######################
class EvaluationSet_lidar:
    # Evaluation set consisting of map and query elements
    def __init__(self, query_set: List[EvaluationTuple_lidar] = None, map_set: List[EvaluationTuple_lidar] = None):
        self.query_set = query_set
        self.map_set = map_set
    ...
    def load(self, pickle_filepath: str):
        ...
        self.query_set = []
        for e in query_l:
            self.query_set.append(EvaluationTuple_lidar(e[0], e[1]))

        self.map_set = []
        for e in map_l:
            self.map_set.append(EvaluationTuple_lidar(e[0], e[1]))
